...
def make_testlist():
    """Build the mapping of all tests."""
    tests = TestDict()
    ...
    for basedir in [TESTS_DIR, GENERATED_TESTS_DIR]:
        for dirpath, _, filenames in os.walk(basedir):
            ...
            for filename in filenames:
                ...
                assert group not in tests, group
    ...
    for group, files in six.iteritems(shader_tests):
        assert group not in tests, 'duplicate group: {}'.format(group)
        ...
        if len(files) == 1:
            ...
            tests[group] = ShaderTest(files[0])
        else:
            tests[group] = MultiShaderTest(files)
    ...
    for basedir in [TESTS_DIR, GENERATED_TESTS_DIR]:
        ...
        for dirpath, _, filenames in os.walk(_basedir):
            ...
            for filename in filenames:
                ...
                tests[group] = ASMParserTest(
                    ['asmparsertest', type_, os.path.join(path, filename)])
    ...
    with tests.group_manager(GleanTest, 'glean') as g:
        g('basic')
        g('api2')
        g('makeCurrent')
        g('bufferObject')
    ...
    for pairs in [(['glsl1'], glean_glsl_tests),
        ...
        for prefix, name in itertools.product(*pairs):
            ...
            tests[groupname] = GleanTest(prefix)
            tests[groupname].env['PIGLIT_TEST'] = name

    with tests.group_manager(PiglitGLTest, 'security') as g:
        g(['initialized-texmemory'], run_concurrent=False)
        g(['initialized-fbo'], run_concurrent=False)
        g(['initialized-vbo'], run_concurrent=False)

    with tests.group_manager(PiglitGLTest, 'shaders') as g:
    ...
    with tests.group_manager(
        ...
        add_msaa_visual_plain_tests(g, ['glx-copy-sub-buffer'],
                                    run_concurrent=False)
    tests[grouptools.join('glx', 'glx-buffer-age vblank_mode=0')] = \
        PiglitGLTest(['glx-buffer-age'],
                     require_platforms=['glx', 'mixed_glx_egl'],
                     run_concurrent=False)
    tests[grouptools.join(
    ...
    with tests.group_manager(PiglitGLTest, 'hiz') as g:
    ...
    with tests.group_manager(PiglitGLTest, 'fast_color_clear') as g:
    ...
    if platform.system() is 'Windows':
        tests.filters.append(lambda p, _: not p.startswith('glx'))

    return tests


profile = XMLProfile('tests/all.profile.xml')  # pylint: disable=invalid-name
